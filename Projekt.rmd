...
Staramy się dobrać węzły spline'a tak aby pokrywały przedziały o największej przypadającej na nie ilości mieszkań:
```{r}
library(splines)
dir_spline <- list()
...
Wynik funkcji `summary` wskazuje na użyteczność takiego podziału (kolumna p-wartości). Zwiększył się również wskaźnik R^2 z 0.8844 do 0.9056, co jest dobrym znakiem.\
...
Jak widać RMSE zbioru na zbiorze testowym zmniejszyło się z około 37100 do ok. 34100. Oznacza to że model nie jest przeuczony (nie ma overfittingu).\
Niektóre spośród dopasowań:
